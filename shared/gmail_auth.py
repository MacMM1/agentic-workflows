...
Run directly to authenticate: python shared/gmail_auth.py
"""
...
def get_credentials(credentials_path: Path | None = None, token_path: Path | None = None) -> Credentials:
    ...
    if not token_path.exists():
        raise FileNotFoundError(
            f"{token_path} not found. Run `python shared/gmail_auth.py` once to authenticate."
        )
...
def bootstrap() -> None:
    ...
    if not credentials_path.exists():
        print(
            ...
            f"{credentials_path}. See newsletter/newsletter.md for the full walkthrough."
        )
        sys.exit(1)
